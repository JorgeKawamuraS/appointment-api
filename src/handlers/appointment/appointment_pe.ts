...
export const handler = async (event: SQSEvent) => {
    console.log("Evento recibido en lambda appointment_pe")
    console.log(event)
    if(event.Records[0].body){
        ...
        const connection = await getConnection(process.env.DB_PE);
        ...
        try{
            ...
            console.log("Error al hacer insert en la bd " + process.env.DB_PE);
            console.log(error)
            return;
        }


        console.log("El appointment con id " + appointment.insuredId +" llegó a appointment_pe")
